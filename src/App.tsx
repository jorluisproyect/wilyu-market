...
function pdf(order:Order,products:Product[],payment:string,cs:CurrencySettings){
  const clean=(v:string)=>v.normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^\x20-\x7E]/g,' ')
  const esc=(v:string)=>clean(v).replace(/\\/g,'\\\\').replace(/\(/g,'\\(').replace(/\)/g,'\\)')
  const lines:string[]=[
    'WILYU MARKET','NOTA DE PEDIDO / ENTREGA',`Pedido: ${order.id}`,
    `Fecha: ${new Date(order.createdAt).toLocaleString()}`,`Cliente: ${order.customerName}`,
    `Telefono: ${order.phone}`,`Metodo: ${payment}`,'','Productos:'
  ]
  order.items.forEach(line=>{const p=products.find(x=>x.id===line.productId);if(p)lines.push(`${line.quantity} x ${p.name}    ${money(p.salePrice*line.quantity)}`)})
  lines.push('',`TOTAL: ${money(order.total)}`)
  cs.enabled.filter(c=>c!=='USD').forEach(c=>lines.push(`${c}: ${money(convert(order.total,c,cs),c)}`))
  lines.push('','Conserve su codigo para consultar el estado del pedido.')
  let y=790
  const content=['BT','/F1 11 Tf']
  lines.forEach((line,i)=>{const size=i===0?18:i===1?14:10;content.push(`/F1 ${size} Tf`,`50 ${y} Td`,`(${esc(line)}) Tj`,`0 0 Td`);y-=i===0?26:i===1?24:17})
  content.push('ET')
  ...
  let out='%PDF-1.4\n', offsets=[0]
  objs.forEach((obj,i)=>{offsets.push(out.length);out+=`${i+1} 0 obj\n${obj}\nendobj\n`})
  ...
  a.href=url;a.download=`Wilyu-${order.id}.pdf`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)
}
...
function Categories({categories,setCategories,remote,refresh}:any){
 ...
 const add=async()=>{if(!n.trim())return;setBusy(true);try{if(remote){const r=await createCategory({name:n.trim(),emoji:e||'🛍️'});if(r.error){alert('No se pudo crear: '+r.error.message);return}await refresh()}else setCategories((x:Category[])=>[...x,{id:uid('cat'),name:n,emoji:e}]);sn('')}finally{setBusy(false)}}
 const remove=async(id:string)=>{if(!confirm('¿Eliminar categoría?'))return;if(remote){const r=await deleteCategory(id);if(r.error){alert('No se pudo eliminar: '+r.error.message);return}await refresh()}else setCategories((x:Category[])=>x.filter(a=>a.id!==id))}
...
function Providers({providers,setProviders,remote}:any){
 ...
 const add=async()=>{if(!n.trim())return;setBusy(true);if(remote){const r=await createProvider({name:n.trim(),phone:p});if(r.error){alert(r.error.message);setBusy(false);return}await refresh()}else setProviders((x:Provider[])=>[...x,{id:uid('prov'),name:n,phone:p}]);sn('');sp('');setBusy(false)}
 const remove=async(id:string)=>{if(!confirm('¿Eliminar proveedor?'))return;if(remote){const r=await deleteProvider(id);if(r.error){alert(r.error.message);return}await refresh()}else setProviders((a:Provider[])=>a.filter(b=>b.id!==id))}
...
function Payments({payments,setPayments,remote}:any){
 ...
 if(ed)return <PayEdit p0={ed} back={()=>setEd(null)} save={async(p:PaymentMethod)=>{if(remote){const r=await savePaymentMethod(p);if(r.error){alert(r.error.message);return}await refresh()}else setPayments((x:PaymentMethod[])=>p.id?x.map(a=>a.id===p.id?p:a):[...x,{...p,id:uid('pay')}]);setEd(null)}}/>
...
function Currencies({cs,setCs,remote}:any){
 ...
 const save=async()=>{if(remote){const r=await saveRates(s.rates);if(r.error){setMsg(r.error.message);return}}setCs(s);setMsg('Tasas guardadas correctamente.')}
